...
- Required artifact: `singularity/work-items/spec-filter/artifacts/specification/spec.md`
...
- <requirement>. *(S1)* [spec-filter:REQ-001]
- <requirement>. *(S1, S2)* [spec-filter:REQ-002]

Acceptance criteria use the same stable, namespaced form:

- <observable acceptance outcome>. *(S1)* [spec-filter:AC-001]
...
Use governed requirement anchors here too (for example `[spec-filter:REQ-003]`); `NFR-001` by
...
1 documentation path(s) and 50 source path(s) were indexed. Product intent must be confirmed from governed requirements.
