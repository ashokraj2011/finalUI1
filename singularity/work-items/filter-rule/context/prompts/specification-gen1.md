...
- Work-item directory: `singularity/work-items/filter-rule`
- Required artifact: `singularity/work-items/filter-rule/artifacts/specification/spec.md`
...
- <requirement>. *(S1)* [filter-rule:REQ-001]
- <requirement>. *(S1, S2)* [filter-rule:REQ-002]

Acceptance criteria use the same stable, namespaced form:

- <observable acceptance outcome>. *(S1)* [filter-rule:AC-001]
...
Use governed requirement anchors here too (for example `[filter-rule:REQ-003]`); `NFR-001` by
...
- Immutable source: `singularity/work-items/filter-rule/source.json`
- SHA-256: `c4bfeb5fe4acd854ae5a1376c6f0927835d9012c2c6b90ea6a8c1ed3790ddaeb`
...
```json
{
  ...
  "id": "filter-rule",
  "title": "filter rule",
  "description": "The rule authoring page the \"visual designer\" ,Need to add a block for filtering .This filtering occurs before the rule evaluation in rule engine .The filter can have conditions ,order by certain atrributes and take the first,last or some range ( both inclusive ) .",
